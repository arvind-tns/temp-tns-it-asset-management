...
describe('allocationGuard', () => {
  ...
  beforeEach(() => {
    ...
    state = { url: '/allocation/assign/123' } as RouterStateSnapshot;
  });
  ...
  it('should redirect to login when user is not authenticated', () => {
    ...
    expect(router.navigate).toHaveBeenCalledWith(
      ['/login'],
      { queryParams: { returnUrl: '/allocation/assign/123' } }
    );
  });

  it('should allow access for user with multiple roles including ADMINISTRATOR', () => {
    const mockUser = {
      id: '4',
      username: 'superuser',
